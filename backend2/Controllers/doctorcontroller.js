    ...
    try {
        if(req.params.doctorid){
            ...
            if(req.body.name && req.body.Charge && req.body.pfp){
                await experts.updateOne({_id:req.params.doctorid},{Name:req.body.name,Charge:req.body.Charge,pfp:req.body.pfp});
            }
            res.redirect('/expert/expertprofile');   
         }
         else{
...
module.exports.getexpertprofile=async (req,res)=>{
    ...
    try {
        if(req.params.doctorid){
            navactive=[1,0,0,0,0,0];
            const doc = await experts.findById(req.params.doctorid);
